...
`CARRIES_COMPLIANCE_FINDING__compliance_core`, never a github_core rival: `github_repository` → finding
...
plugin**, not a later improvement. Found in review on PR# 161 and recorded here rather than left to the pull request, because the pull request is not what somebody reads before arming.
...
| req-github-core-falsifier-reach-5 | `all` Is One Account's Boundary | In Development | An installation reporting `repository_selection: all` holds every repository of the account it is installed on, and no other. A repository under a different owner is outside the installation, so its 404 is not evidence of absence. An installation that reports `all` but names no account has no boundary to test and its reach is `unknown`. | `tests/test_falsifiers.py::TestReachBoundary`. Found in review on PR# 161. |
| req-github-core-falsifier-reach-6 | Reach Belongs To The Run | In Development | A reach resolved from the credential is cached against the lifecycle batch id, not the falsifier instance: an installation narrowed between two runs must be re-read, never carried forward. | `tests/test_falsifiers.py::TestReachIsScopedToTheRun` reuses one falsifier across two runs and narrows the installation between them. Found in review on PR# 161. |
| req-github-core-falsifier-reach-7 | A Repository's Reach Is Read Twice | In Development | A repository has no parent to probe, so before its 404 may retire it the reach is read a second time, after the probe, from a freshly minted credential; both readings must hold. One extra read per run, on that path only. Core's verb cannot cover this: it rejects a verdict whose target was re-observed, and a repository that left the reach is the one this run cannot observe. | `tests/test_falsifiers.py::TestReachIsReadAgainAfterTheProbe`: a narrowing between the two readings refuses the retirement; a stable installation still drops; two absences in one run cost one extra read. Found in review on PR# 161. |
...
asset edges above become `CARRIES_COMPLIANCE_FINDING__compliance_core` pairs from the `github_action`
