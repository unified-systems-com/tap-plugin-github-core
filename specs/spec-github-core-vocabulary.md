...
| `dependabot_alert` | partial | self | proposed (2026-09-03; reshaped 2026-09-09) | GitHub's own vulnerable-dependency finding; CONSUME per the prior-art verdict. Under the 2026-09-09 ruling it is the DETAIL node behind a `compliance_core__compliance_finding` via `DETAILS_FINDING`, and its asset joins to the action and the workflow it flags become `HAS_COMPLIANCE_FINDING` pairs (`req-github-core-dependabot-alerts`). Neutral shape is *finding* — and the finding is now compliance_core's; the source is GitHub's |
