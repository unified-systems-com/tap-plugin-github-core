...
Derived: `uuid5(ns, "BYPASSED__github_core:<suite_uuid>:<ruleset_uuid>")`. One suite may bypass several rules belonging to the same ruleset; that is one edge, with the first rule type on it and the full list on the suite's `bypassed_rules`.
