...
It is also the edge that makes "who bypasses gates around here" a graph question rather than a report. One account with `BYPASSED` edges across nineteen repositories is a different picture from nineteen accounts with one each, and only the graph shows the difference at a glance.
