...
- **Dimensions:** `git.host` only. Alone among this plugin's edges it carries **no** `github.observation` value, and that is **deliberate**. Its source union spans a workflow (declaration) plus a run and a job (execution), so no single value is true for more than a third of the edges it emits. The layer is a property of the *source endpoint*, not of the edge type — so enrichment should stamp each emitted edge from its own source's default rather than the type carrying one. See [`github.observation`](dimensions/github.observation.md).
