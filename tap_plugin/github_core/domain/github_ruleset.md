...
**One question is open, and the obvious test cannot answer it.** GraphQL exposes `bypassActors` on `RepositoryRuleset` — the field REST gates — as `bypassActors(first:) { nodes { bypassMode repositoryRoleDatabaseId actor } }`. Whether it gates that field the way REST does is unresolved, and it matters: a `FORBIDDEN` entry in the `errors` array is detectable and lets a view honestly render *not-observable*, while a silent `[]` is indistinguishable from "nobody can bypass" and would make GraphQL **worse** than REST, which omits the field detectably.

**Why it is unresolved is the useful part. Every ruleset in our own organisation has genuinely zero bypass actors** — verified 2026-08-27 with a privileged token across all four rulesets on the reference repository, each returning `bypass_actors: []` and `current_user_can_bypass: "never"`. When the true answer is empty, "empty" and "withheld" are the same bytes. So every measurement taken here so far — REST and GraphQL, privileged and unprivileged alike — has been run against a case that **cannot discriminate**, and a `200` with `[]` from any credential proves nothing either way. A non-owner PAT was tried against GraphQL and returned `200`, no `errors`, `[]`; that is not evidence of an ungated field, only evidence that this organisation cannot settle the question.

**The discriminating experiment** is a ruleset that actually carries a bypass actor, read with each credential in turn — above all the read-only App, since [the App is the product credential](../../specs/spec-github-core-v0.md) (`req-github-core-app-auth`) and a fine-grained PAT is a first-look path, not what an adopter runs. Until that is done: do not populate a bypass view from GraphQL, and do not read an empty `bypassActors` from any source as evidence of no exemptions.

**Two things that *are* settled by the same pass.** The `403` on `/rulesets/{id}/history` is **not App-specific** — a fine-grained PAT gets it too, with the message *"Resource not accessible by personal access token"* — so `tap#192` is a broader limitation than first recorded. And `current_user_can_bypass` is returned to every credential tried, reading `never` throughout: it reliably answers "can *this* credential bypass" even when the actor list cannot be seen, which is one honest row where the full list is unavailable.
...
- `source_name` — the organisation or repository the ruleset is defined on. With `source_type` it says where to administer the gate, and it is the disambiguator in the fallback key if `databaseId` uniqueness ever fails.
- `configuration` — JSONB for the rules themselves and the ref conditions that target them. Un-schema'd today, which is the same cost noted on [`github_workflow`](github_workflow.md): a blob cannot be queried the way a node can, and required status checks in particular are a corpus concept (`REQUIRES_CHECK` → `status_check`) waiting to come out of it.
