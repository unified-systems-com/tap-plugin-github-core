...
**A live consequence for the collector.** The config layer already runs on GraphQL (`req-github-core-graphql-config`), and its ruleset selection today is `databaseId`, `name`, `enforcement`, `target`, `source` — it does **not** select `bypassActors`. Adding it, which is the obvious next step once the `BYPASSES` edge exists, must not happen before this question is settled: if GraphQL silently answers zero to a refused caller, the collector would land "no exemptions" on the grid for every ruleset, with no signal that anything was withheld.
